function [Throttle1,Throttle2,Throttle3,Throttle4] = BB_changePitch(angle)
...
Mass = 15; %in kg
% MaxAcc = 0.9; %m/s^2
MaxPitch = 5; %degrees (change to radians)
...
while true
    ...
    if (Pitch >= -MaxPitch)&&(Pitch <= MaxPitch)
        disp("The drone will travel at a pitch of: " + Pitch + " degrees.")
        break
    elseif Pitch < -MaxPitch
        disp("You have inputted a value exceeding the negative Max Pitch, please try again")
        Pitch = 0;
        break
    elseif Pitch > MaxPitch
        disp("You have inputted a value exceeding the Max Pitch, please try again.")
        Pitch = 0;
        break
    else
...
Motor3 = (Mass*HorizAcc/4)*(1/cos(TiltAngle) + MassToRotDist*sin(TiltAngle)/MotorToRotDist);
Motor4 = Motor3;
Motor1 = ((Mass*HorizAcc)/cos(TiltAngle) - (Motor3 + Motor4))/2;
Motor2 = Motor1;
